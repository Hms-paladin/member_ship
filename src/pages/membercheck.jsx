...
class Member extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      editOpen:false,
      snackbarstate:false,
      imageUrl: "http://18.213.120.48/uploads/memberimage49_1.jpg",
...
handlePreview = async file => {
  console.log("fileList",this.state.imageUrl)
  if (!this.state.imageUrl && !file.preview) {
    file.preview = await this.getBase64(this.state.imageUrl.originFileObj);
  }
  const fileUrl=this.state.imageUrl
  this.setState({
...
handleChange = (e) => {
  ...
  onSubmit = (e) => {
    ...
    console.log("orignailObj",this.state.fileList)
    
    //formData.append('imageArray',this.state.imageUrl[0].originFileObj);
...
{/* Imaging */}
<Grid item xs={4} md={4}>
  {/* {console.log("check",this.state.editDetails.mem_ImagingList.map(val=>val.id=parseInt(val.id)))} */}
  <FormControl  className="form-input"  style={{marginTop:15}}  error={this.state.validations.mem_TypeId.status}>
  <InputLabel>Imaging</InputLabel>
  <Select
  className="form-input"
  value={this.state.editDetails.mem_ImagingList}
  onOpen={this.handleOpen}
  onClose={this.handleClose}
  onChange={e => this.onChangeEdit(e.target.value,'mem_Imaging')}
  >
